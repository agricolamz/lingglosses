...
```{r, eval = FALSE}
# install.packages("remotes") # in case you do not have this package
...
Unfortunately, glossing extraction implemented in `lingglosses` is case sensitive. That makes it hard to use for the Sign Languages glossing, because 

1) Sign linguists used to gloss with capitalized English translations;
2) Sign language glosses sometimes are splited into two lines associated with two hands.
...
5) There can be some placeholders that corresponds to utterance by first hand, which
are held stationary in the signing space during the articulation made by another hand.

```{r}
gloss_example("{LH}: {CHAIR} ---",
              "{RH}: {} CL:{SIT}.{ON}",
