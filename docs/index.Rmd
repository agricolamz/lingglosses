...
It is also possible to use pictures in a transliteration line, see example from Kazakh-Russian Sign Language [@kuznetsova21: 51]:
...
The first line correpsonds to pictures in markdown format that should be located in the same folder (otherwise you need to specify the path to them, e. g. `![](images/your_plot.png)`). The next three lines correspond to different lines in the example with some non-manual articulation: as before all glossing lines stored as a vector of strings.
