...
* there is also some abbreviation (`r small_caps("sbjv")`, `r small_caps("imp")`) in the article that are absent in the abbreviation list.
...
Unfortunately, some glosses can have multiple meaning in different traditions (e. g. `r small_caps("ass")` can be either associative plural or assertive mood). By default `make_gloss_list()` shows only some entries that were chosen by the package author. You can see all possibilities, if you add argument `all_possible_variants = TRUE`. As you can see, there are two possible values for `r small_caps("prf")` that are listed below:
